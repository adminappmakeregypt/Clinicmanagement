...
const AUDIT_KEY = clinicKey('clinic_audit_v1');
const USER_KEY = clinicKey('clinic_current_user');
...
let currentUser = (window.BMDAuth && BMDAuth.currentUser && (BMDAuth.currentUser().name || BMDAuth.currentUser().email)) 
...
function uniqueSpecialties() {
  const fromDoctors = [...new Set(doctors.map(d => d.specialty).filter(Boolean))];
  const fromMajors = JSON.parse(localStorage.getItem(MAJORS_KEY) || '[]');
  return [...new Set([...fromDoctors, ...fromMajors])];
}
...
saveBtn.addEventListener('click', () => {
  ...
  const data = readForm();
  if (editingId) {
    // keep website-booking link fields (source, websiteRef, websiteSlot, doctorId)
    bookings = bookings.map(b => b.id === editingId
      ? Object.assign({}, b, data, b.doctor !== data.doctor ? { doctorId: '' } : {})
      : b);
...
function deleteBooking(id) {
  ...
  // a deleted website booking frees its online slot (explicit staff action)
  if (b && b.source === 'website' && typeof window.releaseWebsiteSlot === 'function') window.releaseWebsiteSlot(b);
  renderTable();
  toast('تم حذف الحجز');
}
...
function renderTable() {
  ...
  tbody.innerHTML = rows.map(b => {
    ...
    return `
    <tr>
      <td>${escape(b.fullName)}${b.source === 'website' ? ' <span title="حجز من موقع الطبيب" style="font-size:11px;background:#e6f4f1;color:#0f766e;border-radius:999px;padding:1px 7px;white-space:nowrap">🌐 الموقع</span>' : ''}</td>
...
// ============ Live refresh (e.g. a new booking from the doctor website) ============
document.addEventListener('cloud-sync-updated', (e) => {
  if (!e.detail || e.detail.key !== 'clinic_bookings_v1') return;
