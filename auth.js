...
// read localStorage at module-load time (admin.js, patients.js, reports.js,
// script.js) always hit their own clinic's bucket — not a shared "__anon__"
// bucket that would leak data across clinics.
// We remember the last signed-in clinicId in localStorage under a fixed key,
// and fall back to "__anon__" only on the very first visit (before login,
// while the login page is shown).
const LAST_CLINIC_KEY = "bmd::lastClinicId";
...
onAuthStateChanged(auth, (user) => {
  ...
  try { localStorage.setItem(LAST_CLINIC_KEY, mapped.clinicId); } catch {}
  if (bootClinicId !== mapped.clinicId && !isLoginPage) {
    location.reload();
    return;
  }
