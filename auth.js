...
// You can also give any single user their own password by adding
// `password: "something"` on that user's entry.
(function () {
  ...
  function signIn(email, password) {
    ...
    const expected = acc.password || DEFAULT_PASSWORD;
    if (password !== expected) throw new Error('كلمة المرور غير صحيحة');
    const session = {
      email,
      role: acc.role,
  ...
  if (!isLoginPage) {
    ...
    const adminOnlyPages = ['admin.html','reports.html'];
  ...
  if (!isLoginPage && session) {
    document.addEventListener('DOMContentLoaded', function () {
      ...
      if (session.role !== 'admin') {
        document.querySelectorAll('a[href="admin.html"], a[href="reports.html"]').forEach(el => { el.style.display = 'none'; });
      }
    });
  }
})();
